...
def cal_performance(pred, gold, smoothing=True):
...
def show_prediction(seq_probs, labels, vocab):
    ...
    print(' \n')
    print('predicted keywords', [vocab[str(widx.cpu().numpy())] for widx in pred_idx if widx != 0])
    print('gt', [vocab[str(word.cpu().numpy())] for word in labels[0] if word != 0])


def test_show_prediction(seq_probs, labels, vocab):
    '''
            :return: predicted words and GT words.
        '''
    # Print out the predicted sentences and GT
    ...
    for word in labels[0]:
        if word == 3:
            break
        if word != 0:
            gt_words.append(vocab[str(word.cpu().numpy())])
    for widx in pred_idx:
        if widx == 3:
            break
        if widx != 0:
            predicted_words.append(vocab[str(widx.cpu().numpy())])
    print(' \n')
    print('predicted keywords', predicted_words)
    print('gt keywords', gt_words[1:])
...
def convert_caption_labels(captions, vocab, max_len):
    ...
    for batch_idx, cap in enumerate(captions):
        ...
        if len(non_zero[0]) != 0:
            cap_mask[batch_idx][:int(non_zero[0][0])] = 1
        else:
            cap_mask[batch_idx] += 1
