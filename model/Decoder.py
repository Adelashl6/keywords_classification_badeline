...
import numpy as np
from model.transformer import Constants as Constants
from model.transformer.Layers import DecoderLayer


def get_non_pad_mask(seq):
    assert seq.dim() == 2
    return seq.ne(Constants.PAD).type(torch.float).unsqueeze(-1).cuda()  #not equal
...
class Decoder(nn.Module):
    ...
    def forward(self, tgt_seq, tgt_pos, enc_output, return_attns=False):
        ...
        src_tmp = torch.ones(enc_output.shape[0], enc_output.shape[1]).cuda()
